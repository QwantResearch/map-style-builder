...
module.exports = function (style, options) {
  ...
  if (buildSprites) {
    const pinsPromise = options.pins ? pins(options) : Promise.resolve([]);
    pinsPromise.then(pinsPaths => {
      const iconFiles = glob.sync(path.resolve(path.join(styleDir, 'icons', '*.svg')))
        .filter(iconFile => !iconFile.endsWith('-15.svg')); // don't include large icons in sprite
      sprites(options, pinsPaths.concat(iconFiles));
    })
  }
